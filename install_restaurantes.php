<?php
/**
 * install_restaurantes.php
 * ─────────────────────────────────────────────────────────────────────────────
 * Script de instalación LOCAL para crear las tablas y cargar los datos de
 * bares y restaurantes en la base de datos de desarrollo.
 *
 * ⚠️  SOLO EJECUTAR EN LOCAL. NO SUBIR A PRODUCCIÓN SIN AUTORIZACIÓN.
 *
 * Uso: http://localhost/moratalla-murcia_2026/install_restaurantes.php
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
function runSQL(PDO $pdo, string $sql, string $label, array &$log, array &$errors): void {
    try {
        $pdo->exec($sql);
        $log[] = "✅ $label";
...
", "Tabla `restaurantes` creada", $log, $errors);

// ─── 2. CREAR TABLA restaurante_images ────────────────────────────────────
runSQL($pdo, "
...
", "Tabla `restaurante_images` creada", $log, $errors);

// ─── 3. INSERTAR DATOS ────────────────────────────────────────────────────
$count_before = (int)$pdo->query("SELECT COUNT(*) FROM restaurantes")->fetchColumn();

if ($count_before > 0) {
    $log[] = "ℹ️  Ya existen $count_before registros en `restaurantes`. Omitiendo inserción de datos iniciales para evitar duplicados.";
} else {
    $datos = [
    ...
    foreach ($datos as $d) {
        try {
            ...
            $errors[] = "❌ Error insertando '{$d[0]}': " . $e->getMessage();
        }
    }
    $log[] = "✅ Insertados $inserted registros en `restaurantes`.";
}
...
if (!is_dir($uploadDir)) {
    ...
    $log[] = "✅ Directorio `uploads/restaurantes/` creado.";
} else {
    $log[] = "ℹ️  Directorio `uploads/restaurantes/` ya existía.";
}
...
<head>
    <meta charset="UTF-8">
    <title>Instalación Bares y Restaurantes</title>
    <style>
        body { font-family: monospace; background: #111; color: #0f0; padding: 2rem; font-size: 1rem; line-height: 1.8; }
        h1 { color: #fff; border-bottom: 1px solid #333; padding-bottom: 0.5rem; }
        .error { color: #f66; }
        .section { margin: 1.5rem 0; padding: 1rem; border: 1px solid #333; border-radius: 6px; }
        a { color: #6cf; }
        .warning { color: #fa0; background: #1a1000; padding: 1rem; border-left: 3px solid #fa0; margin: 1rem 0; }
    </style>
</head>
<body>
<h1>🍽️ Instalación: Bares y Restaurantes</h1>

<div class="warning">
    ⚠️ Este script es SOLO para instalación LOCAL. No ejecutar en producción sin autorización.
</div>

<div class="section">
    <h2>Log de ejecución:</h2>
    <?php foreach ($log as $line) echo "<p>$line</p>"; ?>
    <?php foreach ($errors as $line) echo "<p class='error'>$line</p>"; ?>
</div>
...
<div class="section">
    ...
    <?php
    try {
        $total = $pdo->query("SELECT COUNT(*) FROM restaurantes")->fetchColumn();
        $totalImg = $pdo->query("SELECT COUNT(*) FROM restaurante_images")->fetchColumn();
        echo "<p>✅ Restaurantes en BD: <strong>$total</strong></p>";
        echo "<p>✅ Imágenes en BD: <strong>$totalImg</strong></p>";
    } catch (PDOException $e) {
...
<div class="section">
    ...
    <p>→ <a href="restaurantes.php">Ver la página pública de Bares y Restaurantes</a></p>
    <p>→ <a href="admin/restaurantes.php">Ir al Panel de Administración</a></p>
    <p>→ Sube fotos desde el admin para cada establecimiento.</p>
    <p>→ Cuando esté todo listo, ejecuta las migraciones 081 y 082 en producción.</p>
</div>
</body>
</html>
