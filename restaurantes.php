...
if (isset($_GET['api']) && $_GET['api'] === 'galeria' && isset($_GET['id'])) {
    ...
    $imgs = $pdo->prepare("SELECT image_path, caption, is_video FROM restaurante_images WHERE restaurante_id = ? AND is_visible = 1 ORDER BY sort_order ASC, id ASC");
    $imgs->execute([$rid]);
    $result = $imgs->fetchAll();
    $out = array_map(fn($i) => ['src' => $i['image_path'], 'caption' => $i['caption'], 'nombre' => $restauranteNombre, 'is_video' => (int)$i['is_video']], $result);
